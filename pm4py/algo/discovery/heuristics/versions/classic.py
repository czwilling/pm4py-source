...
def apply_heu_dfg(
    ...
    dependency_thresh = (
        parameters[defaults.DEPENDENCY_THRESH]
        if defaults.DEPENDENCY_THRESH in parameters
        else defaults.DEFAULT_DEPENDENCY_THRESH
    )
    and_measure_thresh = (
        parameters[defaults.AND_MEASURE_THRESH]
        if defaults.AND_MEASURE_THRESH in parameters
        else defaults.DEFAULT_AND_MEASURE_THRESH
    )
    min_act_count = (
        parameters[defaults.MIN_ACT_COUNT]
        if defaults.MIN_ACT_COUNT in parameters
        else defaults.DEFAULT_MIN_ACT_COUNT
    )
    min_dfg_occurrences = (
        parameters[defaults.MIN_DFG_OCCURRENCES]
        if defaults.MIN_DFG_OCCURRENCES in parameters
        else defaults.DEFAULT_MIN_DFG_OCCURRENCES
    )
    dfg_pre_cleaning_noise_thresh = (
        parameters[defaults.DFG_PRE_CLEANING_NOISE_THRESH]
        if defaults.DFG_PRE_CLEANING_NOISE_THRESH in parameters
        else defaults.DEFAULT_DFG_PRE_CLEANING_NOISE_THRESH
    )
    loops_length_two_thresh = (
        parameters[defaults.LOOP_LENGTH_TWO_THRESH]
